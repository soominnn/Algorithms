...
//     let count = 0;
//     let queue = [[1, 0]];
//     let roads = Array.from(Array(N + 1), () => []);
...
//     //마을 별 연결관계 구하기
//     road.forEach(([src, dest, dis]) => {
//         roads[src].push([dest, dis]);
//         roads[dest].push([src, dis]);
//     })
    
//     while (queue.length > 0) {
//         let [s, d] = queue.shift();
//         for (let i = 0; i < roads[s].length; i++) {
//             let [dest, dis] = roads[s][i];
//             if (!visited[dest]) {
//                 if (distance[dest] > dis + d) {
//                     distance[dest] = dis + d;
//                 }
//                 queue.push([dest, dis + d]);
//                 visited[dest] = true;
//             }
//         }
//     }

//     distance.forEach((country, index) => {if (index > 0 && country <= K) count += 1;})
